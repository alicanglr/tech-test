...
@Service
@Slf4j
public class OnlineService extends AbstractService {

    private final PaymentValidationClient paymentValidationClient;

    public OnlineService(PaymentErrorLogClient logClient, AccountRepository accountRepository,
        PaymentRepository paymentRepository, PaymentValidationClient paymentValidationClient) {
    ...
    public void consumeOnlinePayment(PaymentDto paymentDto) {
        ...
        boolean validPayment = paymentValidationClient.validatePayment(paymentDto);
        ...
        if (validPayment) {
            ...
            logError(PaymentErrorLogDto.builder()
                .payment_id(paymentDto.getPayment_id())
                .error_type(ErrorType.INVALID_PAYMENT.name())
                .error_description("Payment could not validated")
                .build());
        }
    }
